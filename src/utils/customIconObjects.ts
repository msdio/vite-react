...
export const customIconArray: (SimpleIcon & CustomIconProps)[] =
  CustomIconsJson.map((obj) => ({
    ...obj,
    get svg() {
      return (
        PREFIX +
...
CustomIconsJson.forEach((item) => {
  const key =
    "si" +
    item.slug.substring(0, 1).toUpperCase() +
    item.slug.substring(1, item.slug.length);
